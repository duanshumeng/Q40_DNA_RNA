...
cor.ilm <- ddply(ilm_expr, .(group), function(sub_data) {
  if (check_group_size(sub_data$group)) {
    ...
    correlation <- NA  # 如果组中的观测数量不足够，则设置相关性为NA
  }
...
get_fit.plot <- function(data){
  ...
  p.t1 <- ggplot() +
    geom_point(data=data, aes(x = x.Q40, y = y.t1.Q40),color='#4472CA',alpha=0.5) +  # 添加数据点
  ...
  p.t2 <-  ggplot() +
    geom_point(data=data, aes(x = x.Q40, y = y.t2.Q40),color='#4472CA',alpha=0.5) +  # 添加数据点
