...
color_gradient <- colorRampPalette(c("#3783BB", "#FFFFFF")) 
colors <- color_gradient(6)
color_gradient <- colorRampPalette(c('#C94741', "#FFFFFF")) 
colors <- color_gradient(6)

rna_color <- c("#3783BB","#5F9BC8","#87B4D6","#AFCDE3","#D7E6F1","#C94741","#D36B67","#DE908D","#E9B5B3","#F4DAD9")
...
subset(snr_df,Sample=='Quartet') %>%
  group_by(Quality) %>%
  dplyr::summarise(
    Mean_Duplicates = mean(SNR, na.rm = TRUE),   # 计算均值
    SD_Duplicates = sd(SNR, na.rm = TRUE)       # 计算标准差
  )
...
jitter_plot <- function(df,d_type,y_lab,seq_type,ylim,y_position){
  ...
  p1 <- ggplot(df,aes(x=Quality,y=Duplicates,color = Quality,fill=Quality))+
    ...
    geom_half_point(
      ...
      size = 5, range_scale = 0.9,alpha=0.5,shape = 21
    )+
    geom_boxplot(
...
subset(pcc_df,Sample=='Quartet') %>%
  group_by(Quality) %>%
  dplyr::summarise(
    Mean_Duplicates = mean(PCC, na.rm = TRUE),   # 计算均值
    SD_Duplicates = sd(PCC, na.rm = TRUE)       # 计算标准差
  )
...
subset(MCC.df,grepl('Quartet',MCC.df$Sample)) %>%
  group_by(Quality) %>%
  dplyr::summarise(
    Mean_Duplicates = mean(MCC, na.rm = TRUE),   # 计算均值
    SD_Duplicates = sd(MCC, na.rm = TRUE)       # 计算标准差
  )
...
ggplot(T1_df_long, aes(x = Feature, y = Quality, fill = Value)) +
  geom_tile() +
  scale_fill_gradient(low = "white", high = "blue") +  # 设置热图的颜色梯度
  theme_minimal() +
